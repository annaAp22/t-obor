...
window.initCart = function(options) {
    ...
    add = function(id, qnt, callback) {
        qnt = qnt || 1;
        var method = typeof(addMethod) === 'undefined' ? 'get' : window.cart.addMethod;
    ...
    remove = function(id, callback) {
        var method = typeof(removeMethod) === 'undefined' ? 'get' : window.cart.removeMethod;
        if(typeof(window.showLoader) === 'function') window.showLoader();
        $[method](
            window.injectParams(window.cart.removeUrl, { id: id }),
            callback);
    };
