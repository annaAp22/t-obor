                                    ...
                                    @if($article->img)
                                        <div id="field-img-now" class="tab-pane fade active in">
                                            <ul class="ace-thumbnails clearfix">
                                                <li>
                                                    <a href="/{{$article->getImgPath().$article->img}}"  data-rel="colorbox" class="cboxElement">
                                                        <img  src="/{{$article->getImgPreviewPath().$article->img}}">
                                                    </a>
                                                    <div class="tools">
                                                        <a href="{{route('admin.image.crop', ['img' => '/'.$article->getImgPath().$article->img, 'preview' => '/'.$article->getImgPreviewPath().$article->img, 'width' => $article->getPreviewSize('width'), 'height' => $article->getPreviewSize('height')])}}" title="Изменить">
                        ...
                        <div class="col-sm-9">
                            ...
                            <select multiple="" name="categories[]" class="chosen-select form-control tag-input-style" id="form-field-12" data-placeholder="Выберите категорию...">
                                ...
                                @if($category->categories->count()))
                                    @include('admin.articles.dropdown', ['cats' => $category->categories()->orderBy('sort')->get(), 'index' => 1, 'article' => $article])
                                @endif
                                @endforeach
                            </select>
                        </div>
